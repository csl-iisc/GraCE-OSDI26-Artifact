...
def speedup_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    ...
    with maybe_profile(args.export_profiler_trace) as p:
        ...
        with torch.profiler.profile(with_stack=True) as prof:
            for rep in trange(args.repeat, desc="running benchmark"): #range(args.repeat):
                ...
                inputs = (
                    ...
                    else copy.deepcopy(example_inputs)
                    if args.should_clone_inputs
                    else example_inputs
                )
        ...
        prof.export_chrome_trace(f"{output_filename}.json")
    ...
    headers = first_headers + ["median_latency", "mean_latency", "latency_std"]
    row = first_fields + [median[0] * 1000, mean[0] * 1000, std_dev[0] * 1000]
...
class BenchmarkRunner:
    ...
    def run_n_iterations(self, mod, inputs):
        ...
        for _ in range(n - 1):
            # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {_} Start||||||||||||||||||||||||||||||||||||||||")
            # print(f"{inputs=}")
            output = self.model_iter_fn(mod, inputs, collect_outputs=False)
            # print(f"{output=}")
            del output
            # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {_} End  ||||||||||||||||||||||||||||||||||||||||")
        #return self.model_iter_fn(mod, inputs, collect_outputs=True)
        # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {n} Start||||||||||||||||||||||||||||||||||||||||")
        # print(f"{inputs=}")
        output = self.model_iter_fn(mod, inputs, collect_outputs=True)
        # print(f"{output=}")
        # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {n} End  ||||||||||||||||||||||||||||||||||||||||")
        return output
    
    def run_n_iterations_new(self, mod, optimized_iter_fn, inputs):
        n = self.args.iterations
        for _ in range(n - 1):
            # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {_} Start||||||||||||||||||||||||||||||||||||||||")
            # print(f"{inputs=}")
            output = optimized_iter_fn(mod, inputs, collect_outputs=False)
            # print(f"{output=}")
            del output
            # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {_} End  ||||||||||||||||||||||||||||||||||||||||")
        #return self.model_iter_fn(mod, inputs, collect_outputs=True)
        # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {n} Start||||||||||||||||||||||||||||||||||||||||")
        # print(f"{inputs=}")
        output = optimized_iter_fn(mod, inputs, collect_outputs=True)
        # print(f"{output=}")
        # print(f"||||||||||||||||||||||||||||||||||||||||Iteration {n} End  ||||||||||||||||||||||||||||||||||||||||")
        return output
            ...
            try:
                ...
                if self.args.export or self.args.export_aot_inductor:
                    ...
                    with maybe_enable_compiled_autograd(self.args.compiled_autograd):
                        new_result = self.run_n_iterations_new(model_copy, optimized_model_iter_fn, example_inputs)
            except Exception as e:
                log.exception(e)
                print(
    ...
    def dump_accuracy_results_to_file(
        ...
        with self.pick_grad(name, self.args.training):
            ...
            if (
                ...
                # The downside and potential problem, is that the output formats may be different.
                # E.g., the output order might not match, None might be part of output, etc.
            dump_file_name = output_filename.split(".")[0] + "_dynamo_result.pt"
            print("Saving the dynamo result to ", dump_file_name, "...")
            torch.save(new_result, dump_file_name)

            eager_result_dumpt_file_name = output_filename.split(".")[0] + "_eager_result.pt"
            ...
            example_inputs_file = output_filename.split(".")[0] + "_example_inputs.pt"
    ...
    def read_dumped_data_in_files_and_check_accuracy(
        ...
        dump_file_name = output_filename.split(".")[0] + "_dynamo_result.pt"
        dump_file_name_1 = output_filename.split(".")[0] + "_1_dynamo_result.pt"

        eager_result_dump_file_name = output_filename.split(".")[0] + "_eager_result.pt"
        eager_result_dump_file_name_1 = output_filename.split(".")[0] + "_1_eager_result.pt"
        # print("Saving the dynamo result to ", dump_file_name, "...")
        # torch.save(new_result, dump_file_name)
        tolerance, cos_similarity = self.get_tolerance_and_cosine_flag(
                self.args.training, current_device, name
    ...
    def run_performance_test(
        ...
        def warmup(fn, model, example_inputs, mode, niters=5):
            ...
            try:
                ...
                # # if you want to profile the warmup then uncomment the code below
                # # to run the that code inplace of the code below it.
                # fn(model, example_inputs)
                # with torch.profiler.profile(with_stack=True) as prof:
                #     for _ in range(niters-1):
                #         fn(model, example_inputs)
                #         prof.step()
                # prof.export_chrome_trace(f"{output_filename}_warmup.json")
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--should-clone-inputs",
        action="store_true",
        help="Whether to clone the input values before running each time. Dimensions will be kept the same.",
    )
    parser.add_argument(
        "--threads",
        "-t",
