...
def speedup_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    ...
    # # sleep for a bit to let the device cool down
    # print("Sleeping for 10 seconds to let the device cool down...")
    # time.sleep(10)
    ...
    timing_information_per_iter = dict()
    with maybe_profile(args.export_profiler_trace) as p:
        ...
        for rep in trange(args.repeat, desc="running benchmark"):
            #resetting the torch._dynamo.utils.stats
            torch._dynamo.utils.reset_frame_count()
            inputs = (
            ...
            # interleave the runs to handle frequency scaling and load changes
                ...
                ################################################################################
                # dynamo timing related stuff -switching off now
                # data = torch._dynamo.utils.compile_times("csv")
                # if args.disable_cudagraphs:
                #     try:
                #         timing_information_per_iter[data[0][-1]][rep] = eval("["+data[1][-1]+"]")
                #     except:
                #         timing_information_per_iter[data[0][-1]] =dict()
                #         timing_information_per_iter[data[0][-1]][rep] = eval("["+data[1][-1]+"]")
                # else:
                #     for i in range(1, 4):
                #         try:
                #             timing_information_per_iter[data[0][-i]][rep] = eval("["+data[1][-i]+"]")
                #         except:
                #             timing_information_per_iter[data[0][-i]] =dict()
                #             timing_information_per_iter[data[0][-i]][rep] = eval("["+data[1][-i]+"]")
                # try:
                #     timing_information_per_iter["time"][rep] = timings[rep, 0]
                # except:
                #     timing_information_per_iter["time"] = dict()
                #     timing_information_per_iter["time"][rep] = timings[rep, 0]
                ##################################################################################
    ...
    if args.suite:
        first_headers += ["suite"]
        first_fields += [args.suite]

        first_headers += ["mode"]
        
        if args.training:
            first_fields += ['training']
        else:
            first_fields += ['inference']
        
    headers = first_headers + ["median_latency", "mean_latency", "latency_std", "raw_timings"]
    row = first_fields + [median[0] * 1000, mean[0] * 1000, std_dev[0] * 1000, list(timings.squeeze())]
    ...
    output_filename_local_rank = output_filename[:-4] + f"_rank_{local_rank}.csv"
    output_csv(
        output_filename_local_rank,
        headers,
        row,
    )

    # headers, data = torch._dynamo.utils.compile_times(repr="csv", aggregate=False)
    # assert (
    ...
    #########################################################################################
    # dynamo timing related stuffs--- switching off now
    # if args.eager:
    #     return ""
    ...
    # if args.disable_cudagraphs:
    #     print(f"{data[0][-1]} = {data[1][-1]}")
    # else:
    #     print(f"{data[0][-3]} = {data[1][-3]}")
    #     print(f"{data[0][-2]} = {data[1][-2]}")
    #     print(f"{data[0][-1]} = {data[1][-1]}")

    #     # Convert strings to lists of floats
    #     deferred_cudagraph = list(map(float, data[1][-2].split(', ')))
    #     run_graph = list(map(float, data[1][-1].split(', ')))

    #     # Pad the shorter list with zeros
    #     size_diff = len(deferred_cudagraph) - len(run_graph)
    #     run_graph = [0]*(size_diff) + run_graph

    #     # Subtract the second list from the first
    #     result = [a - b for a, b in zip(deferred_cudagraph, run_graph)][size_diff:]
    #     print(f"delta = {result}")
    #     print(f"Overhead = {sum(result)/timings[-1][0]*100}%")
    # print(f"time = {timings[-1][0]}")
    # print(timing_information_per_iter)
    ###############################################################################
...
def get_peak_reserved_memory():
    return torch.cuda.max_memory_reserved() / 10**9
...
class BenchmarkRunner:
    ...
    def check_accuracy(
        ...
        def record_status(accuracy_status, dynamo_start_stats):
            ...
            if current_name in self.non_deterministic_models:
                if accuracy_status in (
                    ...
                    accuracy_status = "pass(model has non determinism)"
        ...
        if name in self.skip_accuracy_checks_large_models_dashboard:
            return record_status("pass_due_to_skip (large models)", dynamo_start_stats=start_stats)
        ...
        with self.pick_grad(name, self.args.training):
            ...
            try:
                ...
                import traceback
                log.warning(
                    "fp64 golden ref were not generated for %s. Setting accuracy check to cosine. Exception (%s): %s\n"
                    "Traceback: %s",
                    name,
                    type(e).__name__,
                    str(e),
                ...
                # commenting the line below just for testing
    ...
    def dump_accuracy_results_to_file(
        self, name, model, example_inputs, optimize_ctx, experiment, tag
    ):
        """
        Checks accuracy.
        ...
        2) Checks if eager itself has variations.
        """
        # example_input_file = '/disk2/abhishek/outputs/XLNetLMHead_tp_2_rank_0_1_example_inputs.pt'
        # print("Loading example_inputs from ", example_input_file)
        # example_inputs = torch.load(example_input_file)
        # for k in example_inputs:
        #     example_inputs[k] = example_inputs[k].to(current_device)

        # example_input_file = '/disk2/abhishek/outputs/nvidia_deeprecommender_tp_2_rank_0_1_example_inputs.pt' 
        # example_input_file = '/disk2/abhishek/outputs/DALLE2_pytorch_tp_2_rank_0_1_example_inputs.pt' 
        # example_input_file = '/disk2/abhishek/outputs/speech_transformer_tp_2_rank_0_1_example_inputs.pt'
        # print("Loading example_inputs from ", example_input_file)
        # example_inputs = torch.load(example_input_file)
        # example_inputs_lst= []
        # for k in example_inputs:
        #     example_inputs_lst.append(k.to(current_device))
        # example_inputs = tuple(example_inputs_lst)
        ...
        with self.pick_grad(name, self.args.training):
            ...
            model_copy = None
            try:
                model_copy = model # self.deepcopy_and_maybe_parallelize(model)
                self.init_optimizer(name, current_device, model_copy.parameters())
                correct_result = self.run_n_iterations(
                    model_copy, clone_inputs(example_inputs)
                )
            ...
            if (
                ...
                # E.g., the output order might not match, None might be part of output, etc.
            local_rank = int(os.environ.get("LOCAL_RANK", "0"))
            dump_file_name = output_filename.split(".")[0] + f"_rank_{local_rank}_dynamo_result.pt"
            print("Saving the dynamo result to ", dump_file_name, "...")
            torch.save(new_result, dump_file_name)

            eager_result_dumpt_file_name = output_filename.split(".")[0] + f"_rank_{local_rank}_eager_result.pt"
            print("Saving the eager result to ", eager_result_dumpt_file_name, "...")
            torch.save(correct_result, eager_result_dumpt_file_name)
            
            # dump the example inputs as well
            example_inputs_file = output_filename.split(".")[0] + f"_rank_{local_rank}_example_inputs.pt"
            print("Saving the example inputs to ", example_inputs_file, "...")
            torch.save(example_inputs, example_inputs_file)
            
            # new_result_loaded = torch.load(dump_file_name)
            # print(same(new_result, new_result_loaded, fp64_outputs, equal_nan=self.equal_nan, cos_similarity=cos_similarity, tol=tolerance))
        return record_status(accuracy_status, dynamo_start_stats=start_stats)
    
    def read_dumped_data_in_files_and_check_accuracy(
        self, name, model, example_inputs, optimize_ctx, experiment, tag
    ):
        dump_file_name = output_filename.split(".")[0] + "_rank_0_dynamo_result.pt"
        dump_file_name_1 = output_filename.split(".")[0] + "_1_rank_0_dynamo_result.pt"

        eager_result_dump_file_name = output_filename.split(".")[0] + "_rank_0_eager_result.pt"
        eager_result_dump_file_name_1 = output_filename.split(".")[0] + "_1_rank_0_eager_result.pt"
        # print("Saving the dynamo result to ", dump_file_name, "...")
        # torch.save(new_result, dump_file_name)
        tolerance, cos_similarity = self.get_tolerance_and_cosine_flag(
                self.args.training, current_device, name
            )
        result_loaded = torch.load(dump_file_name)
        result_loaded_1 = torch.load(dump_file_name_1)

        eager_result_loaded = torch.load(eager_result_dump_file_name)
        eager_result_loaded_1 = torch.load(eager_result_dump_file_name_1)

        with open(dump_file_name+".txt", "w") as f:
            f.write(str(result_loaded))
        with open(dump_file_name_1+".txt", "w") as f:
            f.write(str(result_loaded_1))
        
        with open(eager_result_dump_file_name+".txt", "w") as f:
            f.write(str(eager_result_loaded))
        with open(eager_result_dump_file_name_1+".txt", "w") as f:
            f.write(str(eager_result_loaded_1))
    ...
    def check_tolerance(
        ...
        if name in self.skip_accuracy_checks_large_models_dashboard:
            tolerance_status = "pass_due_to_skip(large model)"
    ...
    def run_performance_test(
        ...
        def warmup(fn, model, example_inputs, mode, niters=5):
            ...
            try:
                ...
                if current_device == "cuda":
                    ...
                    peak_reserved_mem = get_peak_reserved_memory()
            ...
            return latency, peak_mem, dynamo_stats, peak_reserved_mem
        ...
        with self.pick_grad(name, self.args.training):
            ...
            results = []
            if self.args.eager:
                eager_latency, eager_peak_mem, _, eager_peak_reserved_mem = warmup(
                ...
                if self.args.print_memory:
                    ...
                    print(
                        f"reserved memory: eager: {eager_peak_reserved_mem:.2f} GB, "
                    )
            ...
            if not self.args.eager:
                with maybe_enable_compiled_autograd(self.args.compiled_autograd):
                    dynamo_latency, dynamo_peak_mem, dynamo_stats, dynamo_peak_reserved_mem = warmup(
                ...
                compilation_time = dynamo_latency + aot_compilation_time
            # compression_ratio = (
            #     eager_peak_mem / dynamo_peak_mem if dynamo_peak_mem else 0.0
            # )
            if not self.args.eager and self.args.print_memory:
                print(
                    f"dynamo: {dynamo_peak_mem:.2f} GB, "
                    #f"ratio: {compression_ratio:.2f}"
                )
                print(
                    f"dynamo: {dynamo_peak_reserved_mem:.2f} GB, "
                    #f"ratio: {compression_ratio:.2f}"
                )
            ...
            # #### sleep for 1 mins before the experiment call
    ...
    def run_one_model(
        ...
        if self.args.accuracy:
            ...
            print(status)
        elif self.args.dump_accuracy_results:
            self.dump_accuracy_results_to_file(
                name, model, example_inputs, optimize_ctx, experiment, tag
            )
        elif self.args.read_dumped_data_in_files_and_check_accuracy:
            self.read_dumped_data_in_files_and_check_accuracy(
                name, model, example_inputs, optimize_ctx, experiment, tag
            )
        torch.cuda.empty_cache()
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--eager",
        action="store_true",
        help="Runs only the eager mode"
    )

    parser.add_argument(
        "--suite",
        help="Optionally you can show about the suite like Torchbench, TIMM, Huggingface in the output csv"
    )
    ...
    mode_group.add_argument(
        "--dump-accuracy-results",
        action="store_true",
        help="Dump accuracy results to a file",
    )

    mode_group.add_argument(
        "--read-dumped-data-in-files-and-check-accuracy",
        action="store_true",
        help="Read dumped data in files and check accuracy",
    )
...
def run(runner, args, original_dir=None):
    ...
    if args.only and should_diff_branch(args):
        ...
        if output_filename and os.path.exists(output_filename):
            # get the last successful model run along with the batch size
            with open(output_filename, 'r') as file:
                last_line = collections.deque(csv.reader(file), maxlen=1)

            if(len(last_line[0]) ==5 ): # for eager only
                # dev,name,batch_size,abs_latency,eager_peak_mem 
                # checkpointing eager mode
                tmp_dev, tmp_name, tmp_batch_size, tmp_abs_latency, \
                    tmp_dynamo_peak_mem = last_line[0]
            elif (len(last_line[0]) == 10): # for accuracy
                ...
                tmp_dev, tmp_name, tmp_batch_size, tmp_accuracy, tmp_calls_captured, tmp_unique_graphs, \
                    tmp_graph_breaks, tmp_unique_graph_breaks, tmp_autograd_captures, tmp_autograd_compiles = last_line[0]
            else:
                # For performance 
                #dev,name,batch_size,median_latency,mean_latency,latency_std,raw_timings,dynamo_peak_mem,calls_captured,unique_graphs,graph_breaks,unique_graph_breaks,autograd_captures,autograd_compiles
                tmp_dev, tmp_name, tmp_batch_size, tmp_median_latency,tmp_mean_latency,tmp_latency_std,tmp_raw_timings, \
                    tmp_dynamo_peak_mem, tmp_calls_captured, tmp_unique_graphs, \
                    tmp_graph_breaks, tmp_unique_graph_breaks, tmp_autograd_captures, tmp_autograd_compiles = last_line[0]

            # Now you can use the variables
            print(f"Last_Model: {tmp_name}, Last Batch Size: {tmp_batch_size}")
            checkpoint_model_name = tmp_name
            checkpoint_batch_size = int(tmp_batch_size)
        else:
            # no checkpoint found. So start from the beginning
        ...
        for i, name in enumerate(model_names):
            ...
            ########################checkpoint support########################
            if current_name == checkpoint_model_name:
                checkpoint_reached = True
            ...
            while True:
                ############################checkpoint support##################
                if current_name == checkpoint_model_name and my_batch_size <= checkpoint_batch_size:
                    my_batch_size *= 2
                    continue
                #################################################################
                print(f"Running {name} with batch size {my_batch_size}")
                try:
                    ...
                    # # )
                    ######################
                    # Check if the model name starts with 'detectron_'
                    if name.startswith("detectron2_"):
                        # Replace 'bfloat16' with 'amp' in sys.argv
                        modified_args = [arg if arg != '--bfloat16' else '--amp' for arg in sys.argv]
                    else:
                        modified_args = sys.argv

                    result = subprocess.run(
                                    [sys.executable] + modified_args + [f"--only={name}"] + [f"--batch_size={my_batch_size}"], timeout=timeout, capture_output=True, text=True
                                )
                    result.check_returncode()
                    print(result.stdout, file=sys.stderr)
                    print(result.stderr, file=sys.stderr)
                    ###########################
                    # if exceptions are handled properly then OOM does not cause
                    # program crash and exception is not raised
                    # but OOM info is there in the stderr or stdout
                    if ("torch.cuda.OutOfMemoryError" in result.stderr) or \
                        ("CUDA out of memory" in result.stderr) or \
                       ("torch.cuda.OutOfMemoryError" in result.stdout) or \
                        ("CUDA out of memory" in result.stdout):
                        print(f"batch size {my_batch_size} too large for {name}")
                        raise subprocess.CalledProcessError(1, "OOM") # to handle OOM
                    # handling certain cases of models for torchbench where the models fail..
                    elif "eager_fail_to_run" in result.stderr or \
                        "eager_fail_to_run" in result.stdout:
                        raise subprocess.CalledProcessError(1, "eager_fail_to_run")
                    ...
                    #########################################################
                my_batch_size *= 2
                if name == "doctr_reco_predictor" and my_batch_size >= 4096: # because detectron times out
                    print("Had enough of batch_sizes...")
                    break

                if my_batch_size > 65535:
                    print("Had enough of batch_sizes...")
                    break
