...
class HuggingfaceRunner(BenchmarkRunner):
    ...
    def load_model(
        ...
        # model = model.to(device, dtype=dtype)
         # ---- TP injection (optional) ----
        tp_size = int(os.environ.get("TP_SIZE", "1"))
        # flag to enable TP (do it later)
        # if extra_args and "tp_size" in extra_args:
        #     tp_size = int(extra_args["tp_size"])
        if tp_size > 1:
            import torch.distributed as dist
            from tp_inject_xlnet import build_tp_group, apply_xlnet_tensor_parallel
            if not dist.is_initialized():
                # your launcher should set LOCAL_RANK etc.; backend NCCL recommended
                dist.init_process_group(backend="nccl")
            # place model on the right CUDA before we allocate new Parameters
            local_rank = int(os.environ.get("LOCAL_RANK", "0"))
            torch.cuda.set_device(local_rank)
            model = model.to(local_rank, dtype=dtype)
            tp_group = build_tp_group(tp_size)
            model = apply_xlnet_tensor_parallel(model, tp_group)
        else:
            model = model.to(device, dtype=dtype)
...
def _cleanup_dist():
    # make sure pending CUDA work is done (prevents NCCL teardown races)
    try:
        if torch.cuda.is_available():
            torch.cuda.synchronize()
    except Exception:
        pass
    # gracefully tear down the process group
    if dist.is_available() and dist.is_initialized():
        try:
            # optional: keeps ranks aligned; skip if you ever see a hang on failures
            dist.barrier()
        except Exception:
            pass
        try:
...
# run at interpreter exit as well (extra safety)
...
if __name__ == "__main__":
    huggingface_main()
    # ensure the process actually exits (rank 0 and non-0)
