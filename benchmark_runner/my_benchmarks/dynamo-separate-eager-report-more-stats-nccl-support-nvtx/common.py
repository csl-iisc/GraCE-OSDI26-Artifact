    ...
    with maybe_profile(args.export_profiler_trace) as p:
        ...
        with torch.profiler.profile(with_stack=True) as prof:
            for rep in trange(args.repeat, desc="running benchmark"):
                #print(f"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<Start of Iteration::{rep}>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

                # push range for current iteration
                torch.cuda.nvtx.range_push("iteration{}".format(rep))
                
                # # starting the perf page fault counter
                # perf.perf_start()#resetting the torch._dynamo.utils.stats
                ...
                #print(f"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<End of Iteration::{rep}>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                
                # # ending the page fault counter
                # perf.perf_end()
                # pop iteration range
                torch.cuda.nvtx.range_pop()
                prof.step()

                # page_faults = perf.perf_get_page_fault_count()
                # time = timings[rep, 0]
                # print(f"{rep}:({time}, {page_faults})")
        prof.export_chrome_trace(f"{output_filename_local_rank}.json")        
